...
class UPDETRWrapper:
    ...
    def set_support_images(self, support_dir: str):
        ...
        [UPDATED]: Automatically adds ONE extra placeholder slot at the end for dynamic updates.
        """
        ...
        # [UPDATED] Create a list of tensors
        tensor_list = [self.patch_transform(p) for p in patches]
        
        # [UPDATED] Add ONE placeholder. 
        # We copy the first image as the initial value for the placeholder 
        # to ensure the model receives valid data, or use zeros. Copying is usually safer for DETR.
        tensor_list.append(tensor_list[0].clone()) 
        
        # Stack: [N+1, 3, 128, 128] -> [1, N+1, 3, 128, 128]
        self.support_tensors = torch.stack(tensor_list, dim=0).unsqueeze(0).to(self.device)
        print(f"[UPDETR] Support initialized with {len(patches)} static images + 1 dynamic placeholder.")

    def update_dynamic_support(self, image_numpy: np.ndarray):
        """
        [NEW]: Updates the dynamic placeholder (the last element) in the support tensor.
        """
        ...
        # Transform
        new_patch = self.patch_transform(pil_img).to(self.device)
        
        # Update only the last element of the batch
        # self.support_tensors shape is [1, num_patches, 3, H, W]
        self.support_tensors[0, -1] = new_patch
        # print(f"[UPDETR] Dynamic support patch updated.")
...
def refine_support_images(support_dir: str, refined_dir: str, yolo: YOLOWrapper, conf=0.01):
    ...
    for p in paths:
        ...
        if len(dets) == 0:
            ...
            cv2.imwrite(str(Path(refined_dir) / p.name), img)
            continue

        # chọn box lớn nhất (object chính)
        det = max(dets, key=lambda d: (d["bbox"][2]-d["bbox"][0]) * (d["bbox"][3]-d["bbox"][1]))

        crop = crop_with_mask(img, det["bbox"], det.get("mask"))
        if crop is None:
            cv2.imwrite(str(Path(refined_dir) / p.name), img)
        else:
            cv2.imwrite(str(Path(refined_dir) / p.name), crop)
...
def process_video_pipeline(
    ...
    support_dir = refine_support_images(support_dir, support_refined, yolo, conf=conf_yolo_refined)
    ...
    while cap.isOpened():
        ...
        if frame_idx % frame_sample_rate == 0:
            ...
            best_det = None  # [UPDATED] To store the best detection in this frame
            max_sim = -1.0   # [UPDATED] To track max similarity
            ...
            for det in all_detections:
                ...
                # [UPDATED] Check for the best detection
                if sim > max_sim:
                    max_sim = sim
                    best_det = det
            ...
            # [UPDATED] Dynamic Support Update Logic
            # Condition: High similarity (>= 0.4). 
            # "all bbox same place" is implicitly handled by picking the best detection (max_sim)
            # and the fact that UP-DETR outputs cluster around the object.
            if best_det is not None and max_sim >= 0.4:
                # Crop the object again to be sure
                crop_for_update = crop_with_mask(frame, best_det['bbox'], best_det.get('mask'))
                if crop_for_update is not None:
                    updetr.update_dynamic_support(crop_for_update)
...
if __name__ == '__main__':
    ...
    TOTAL_OUTPUT_DIR = Path(f'/mlcv2/WorkingSpace/Personal/quannh/Project/Project/ZaloAI2025/src/idea/{TYPE}1_updetr_mobileclip2_tracker')
    ...
    # Example iterator (adjust glob pattern to match your dataset structure)
